// event-place-images.js — safe event media display + independent modal editor
(function safeEventPlaceMedia(){
  try {
    if (window.__AA_SAFE_EVENT_PLACE_MEDIA_V4__) return;
    window.__AA_SAFE_EVENT_PLACE_MEDIA_V4__ = true;

    var h = window.React && React.createElement;
    var EVENT_DEFS = [
    ...
    function mediaUrl(ev){
      return String((ev && (ev.image || ev.media || ev.video || ev.photo || ev.image_url)) || '').trim();
    }
    ...
    function addStyle(){
      if (document.getElementById('aa-safe-event-media-style-v4')) return;
      var style = document.createElement('style');
      style.id = 'aa-safe-event-media-style-v4';
      style.textContent = `
        ...
        .aa-media-open-btn{margin-left:8px;border:1px solid var(--line);background:#fffaf1;color:var(--ink);border-radius:999px;padding:8px 10px;font-family:var(--sans);font-size:9px;letter-spacing:.16em;text-transform:uppercase;cursor:pointer}
        .aa-media-modal-back{position:fixed;inset:0;z-index:420;background:rgba(63,52,32,.45);backdrop-filter:blur(5px);display:flex;align-items:center;justify-content:center;padding:18px}
        .aa-media-modal{width:min(620px,100%);max-height:88vh;overflow:auto;background:#fffdf8;border:1px solid var(--line);box-shadow:0 26px 90px -42px rgba(0,0,0,.45);padding:22px;display:grid;gap:14px}
        .aa-media-modal-head{display:flex;align-items:center;justify-content:space-between;gap:12px;border-bottom:1px solid var(--line);padding-bottom:12px}
        .aa-media-modal-head h3{margin:0;font-family:var(--display);font-size:18px;letter-spacing:.12em;text-transform:uppercase;color:var(--ink)}
        .aa-media-close{border:0;background:transparent;font-size:22px;cursor:pointer;color:var(--ink-soft)}
        .aa-media-row{display:grid;grid-template-columns:94px 1fr;gap:12px;border:1px solid rgba(95,75,52,.16);background:#fff;padding:10px;border-radius:8px}
        .aa-media-preview{width:94px;height:94px;border:1px solid var(--line);background:var(--paper-2);display:flex;align-items:center;justify-content:center;overflow:hidden;color:var(--ink-mute);font-size:10px;text-align:center;position:relative}
        .aa-media-preview img,.aa-media-preview video{width:100%;height:100%;object-fit:cover;display:block}.aa-media-preview[data-kind='video']:after{content:'▶';position:absolute;right:6px;bottom:6px;background:rgba(0,0,0,.48);color:#fff;width:22px;height:22px;border-radius:999px;display:flex;align-items:center;justify-content:center;font-size:10px}
        ...
        .aa-media-status{min-height:18px;font-size:12px;color:var(--ink-mute);font-family:var(--sans)}
        @media(max-width:720px){.aa-media-row{grid-template-columns:70px 1fr}.aa-media-preview{width:70px;height:70px}.aa-event-place-media{margin-top:6px}}
      `;
    ...
    function EventCardWithSafeMedia(props){
    ...
    if (h && window.Reveal) { try { EventCard = EventCardWithSafeMedia; } catch(e) {} window.EventCard = EventCardWithSafeMedia; }

    async function loadContent(){
      var res = await fetch('/api/content?eventMediaSafe=' + Date.now(), { cache:'no-store' });
    ...
    function previewHtml(ev){ var url=mediaUrl(ev), type=mediaType(ev); if (!url) return '<span>Sin archivo</span>'; return type === 'video' ? '<video src="'+esc(url)+'" muted playsinline preload="metadata"></video>' : '<img src="'+esc(url)+'" alt="">'; }

    async function openEditor(){
      addStyle();
      var data = await loadContent().catch(function(e){ alert('No se pudo cargar el contenido'); throw e; });
      var back = document.createElement('div'); back.className = 'aa-media-modal-back';
      back.innerHTML = '<div class="aa-media-modal"><div class="aa-media-modal-head"><h3>Media eventos</h3><button class="aa-media-close" type="button">×</button></div><div style="font-size:12px;color:var(--ink-mute);line-height:1.5">Puedes usar imagen o video. El video se reproduce automáticamente, sin sonido y en loop.</div><div class="aa-media-list"></div><div class="aa-media-status"></div></div>';
      ...
      function render(){
        list.innerHTML = EVENT_DEFS.map(function(def){ var ev=data[def.key]||{}; var type=mediaType(ev); return '<div class="aa-media-row" data-key="'+def.key+'"><div class="aa-media-preview" data-kind="'+type+'">'+previewHtml(ev)+'</div><div><div class="micro" style="color:var(--sage-deep);font-size:9px;margin-bottom:6px">'+def.label+'</div><input class="aa-media-input" data-role="url" placeholder="URL de imagen o video" value="'+esc(mediaUrl(ev))+'"><div style="display:flex;gap:8px;align-items:center;margin-top:7px"><span style="font-size:11px;color:var(--ink-mute)">Tipo:</span><select class="aa-media-select" data-role="type"><option value="image" '+(type==='image'?'selected':'')+'>Imagen</option><option value="video" '+(type==='video'?'selected':'')+'>Video autoplay</option></select></div><input class="aa-media-input" data-role="caption" placeholder="Pie de foto opcional" style="margin-top:7px" value="'+esc(byLang(ev,'image_caption','es'))+'"><div class="aa-media-actions"><button class="btn" type="button" data-role="upload">Subir imagen/video</button><button class="btn" type="button" data-role="save">Guardar</button><button class="btn" type="button" data-role="clear">Quitar</button></div><input type="file" accept="image/*,video/mp4,video/webm,video/quicktime,video/*" data-role="file" style="display:none"></div></div>'; }).join('');
        list.querySelectorAll('.aa-media-row').forEach(function(row){
          ...
          file.onchange=async function(){
            ...
            catch(e){ console.error(e); setStatus('No se pudo subir. Usa un MP4 corto y ligero.', false); }
            finally { file.value=''; }
          };
          row.querySelector('[data-role=save]').onclick=async function(){
            ...
            catch(e){ console.error(e); setStatus('No se pudo guardar.', false); }
          };
          row.querySelector('[data-role=clear]').onclick=async function(){
            ...
            catch(e){ console.error(e); setStatus('No se pudo quitar.', false); }
          };
        });
      }
      render();
    ...
    function ensureButton(){
      try {
        var hd = document.querySelector('.admin-panel.open .hd');
        if (!hd || hd.querySelector('.aa-media-open-btn')) return;
        var btn = document.createElement('button'); btn.type='button'; btn.className='aa-media-open-btn'; btn.textContent='Media eventos'; btn.onclick=openEditor;
        hd.appendChild(btn);
      } catch(e) {}
    }

    addStyle();
    document.addEventListener('click', function(){ setTimeout(ensureButton, 120); }, true);
    setInterval(ensureButton, 1500);
  } catch(err) {
    console.error('[SafeEventMedia] disabled:', err);
  }
})();