// admin-safe-panel-v6.js — reliable isolated editor for welcome + dress inspiration
(function adminSafePanelV6(){
  try {
    if (window.__AA_ADMIN_SAFE_PANEL_V6__) return;
    window.__AA_ADMIN_SAFE_PANEL_V6__ = true;
    ...
    const DEFAULTS = {
      welcome: {
        ...
        body_es:'Si hoy están aquí, es porque de alguna manera han formado parte de nuestra historia.\n\nGracias por acompañarnos en este momento tan especial.',
    ...
    function normalize(data){
      ...
      ['day1','day2'].forEach(function(day){
        if (!next.dressAdmin[day]) next.dressAdmin[day] = clone(DEFAULTS.dressAdmin[day]);
    ...
    function setBusy(isBusy){ document.querySelectorAll('[data-aa-safe-save],[data-aa-safe-reload],.aa-safe-btn').forEach(function(btn){ if (btn.dataset.aaSafeClear || btn.dataset.aaSafeAdd || btn.dataset.aaSafeSave || btn.dataset.aaSafeReload) btn.disabled = !!isBusy; }); }

    function getAt(path){
      const parts = path.split('.');
      let cur = state;
      for (let i=0;i<parts.length;i++) cur = cur == null ? '' : cur[/^\d+$/.test(parts[i]) ? Number(parts[i]) : parts[i]];
      return cur == null ? '' : cur;
    }
    ...
    async function loadContent(){
      ...
      try {
        ...
        if (!res.ok || !json.ok || !json.data) throw new Error(json.error || 'load_failed');
        state = normalize(json.data);
      } catch(e) {
        console.error('[AdminSafePanelV6] load failed:', e);
    ...
    async function saveContent(){
      if (!state || saving || uploading) return false;
      saving = true;
      setBusy(true);
      ...
      try {
        ...
        const json = await res.json().catch(function(){ return {}; });
        if (!res.ok || !json.ok || json.remoteSaved === false) throw new Error(json.error || 'save_failed');
        state = normalize(json.data || payload);
        dirty = false;
        publishPreview();
        renderEditor(false);
        ...
        console.error('[AdminSafePanelV6] save failed:', e);
    ...
    function imageToDataUrl(file, maxSide, quality){
      return new Promise(function(resolve, reject){
        ...
        reader.onload = function(){
          ...
          img.onerror = function(){
            if (String(originalDataUrl).length < 11000000) resolve(originalDataUrl);
    ...
    async function uploadImage(file){
      const dataUrl = await imageToDataUrl(file, 1500, 0.84);
      const res = await fetch('/api/gallery/upload', {
        method:'POST',
        headers:{ 'Content-Type':'application/json' },
        body:JSON.stringify({ filename:file.name || 'inspiracion.jpg', dataUrl:dataUrl })
      });
      const json = await res.json().catch(function(){ return {}; });
      if (!res.ok || !json.ok || !json.url) throw new Error(json.error || 'upload_failed');
      return json.url;
    }

    function ensureStyle(){
      if (document.getElementById('aa-admin-safe-style-v6')) return;
      const style = document.createElement('style');
      style.id = 'aa-admin-safe-style-v6';
      style.textContent = `
        ...
        .aa-safe-back{position:fixed;inset:0;z-index:440;background:rgba(34,26,14,.46);backdrop-filter:blur(7px);display:flex;align-items:center;justify-content:center;padding:22px;}
        ...
        .aa-safe-row input,.aa-safe-row textarea{width:100%;border:1px solid var(--line);background:#fff;font-family:var(--serif);font-size:14px;padding:8px 10px;color:var(--ink);border-radius:5px;box-sizing:border-box;}
        ...
        .aa-safe-actions input[type=file]{font-size:11px;max-width:100%;}
        ...
        .aa-safe-btn[disabled]{opacity:.55;cursor:not-allowed;}
    ...
    function editorHtml(){
      return '<div class="aa-safe-note">Edita y presiona “Guardar cambios”. Al seleccionar una imagen, se comprime, se sube y se guarda automáticamente.</div>'+ 
    ...
    function bindEditor(root){
      ...
      root.querySelectorAll('[data-aa-safe-file]').forEach(function(input){
        input.addEventListener('change', async function(){
          ...
          try {
            const path = input.getAttribute('data-aa-safe-file');
            const url = await uploadImage(file);
            uploading = false;
            setAt(path, url);
            renderEditor(false);
            setStatus('Imagen subida · guardando…');
            const ok = await saveContent();
            setStatus(ok ? 'Imagen subida y guardada' : 'Imagen subida, falta guardar');
          } catch(e) {
            uploading = false;
            console.error('[AdminSafePanelV6] upload failed:', e);
            setStatus('Error al subir imagen');
            alert('No se pudo subir la imagen. Prueba con una foto JPG/PNG o una imagen de menor tamaño.');
          } finally { input.value = ''; uploading = false; setBusy(false); }
        });
      });
    }
    function renderEditor(resetStatus){
      const body = document.querySelector('.aa-safe-body');
      if (!body) return;
      body.innerHTML = editorHtml();
      bindEditor(body);
      if (resetStatus !== false) setStatus(dirty ? 'Cambios sin guardar' : 'Listo');
    }
    async function openEditor(){
      ensureStyle(); closeEditor();
    ...
    function closeWithPrompt(){ if (dirty && !confirm('Hay cambios sin guardar. ¿Cerrar de todos modos?')) return; closeEditor(); }
    function injectButton(){
      ensureStyle();
      const panel = document.querySelector('.admin-panel');
      const hd = panel && panel.querySelector('.hd');
      if (!hd) return false;
      hd.querySelectorAll('.aa-safe-open-btn').forEach(function(old){ old.remove(); });
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'aa-safe-open-btn';
      btn.setAttribute('data-aa-safe-v6','1');
      ...
      if (close && close.parentNode === hd) hd.insertBefore(btn, close); else hd.appendChild(btn);
      return true;
    }
    function start(){
      let tries = 0;
      const timer = setInterval(function(){ tries += 1; if (injectButton() || tries > 60) clearInterval(timer); }, 250);
  ...
  } catch(err) { console.error('[AdminSafePanelV6] disabled after error:', err); }
})();