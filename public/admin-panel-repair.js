// admin-panel-repair.js — lightweight non-blocking safety layer for admin panel
(function repairAdminPanel(){
  try {
    if (window.__AA_ADMIN_PANEL_REPAIR_V2__) return;
    window.__AA_ADMIN_PANEL_REPAIR_V2__ = true;
    ...
    function addStyle(){
      ...
      style.textContent = `
        .admin-fab,.admin-link,.modal-back,.modal,.admin-panel,.admin-panel *{pointer-events:auto!important;}
        ...
        .admin-panel .tabs button,.admin-panel .ft button,.admin-panel .body button,.modal button{cursor:pointer!important;pointer-events:auto!important;}
        .admin-panel .body input,.admin-panel .body textarea,.admin-panel .body select,.modal input{pointer-events:auto!important;user-select:text!important;}
      `;
    ...
    addStyle();
    var idle = window.requestIdleCallback || function(fn){ return setTimeout(fn, 500); };
    idle(loadSafeSectionsEditor);
