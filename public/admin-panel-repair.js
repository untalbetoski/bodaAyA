...
(function repairAdminPanel(){
  try {
    if (window.__AA_ADMIN_PANEL_REPAIR_V4__) return;
    window.__AA_ADMIN_PANEL_REPAIR_V4__ = true;

    function addStyle(){
      if (document.getElementById('aa-admin-panel-repair-style-v4')) return;
      var style = document.createElement('style');
      style.id = 'aa-admin-panel-repair-style-v4';
      style.textContent = `
    ...
    function ensureAdminAccessHints(){
      try {
        ...
        buttons.forEach(function(btn){
          if (!btn || btn.getAttribute('data-aa-admin-repaired-v4')) return;
          btn.setAttribute('data-aa-admin-repaired-v4','1');
