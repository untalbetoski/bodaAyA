// admin-safe-panel-v5.js — stable isolated editor for extra wedding sections
(function adminSafePanelV5(){
  try {
    if (window.__AA_ADMIN_SAFE_PANEL_V5__) return;
    window.__AA_ADMIN_SAFE_PANEL_V5__ = true;
    ...
    async function loadContent(){
      ...
      try {
        ...
        console.error('[AdminSafePanelV5] load failed:', e);
    ...
    async function saveContent(){
      ...
      try {
        ...
        console.error('[AdminSafePanelV5] save failed:', e);
      ...
      return new Promise(function(resolve, reject){
        ...
        reader.onload = function(){
          ...
          img.onerror = function(){
            if (String(originalDataUrl).length < 12000000) resolve(originalDataUrl);
    ...
    async function uploadImage(file){
      const dataUrl = await imageToDataUrl(file, 1600, 0.86);
    ...
    function ensureStyle(){
      if (document.getElementById('aa-admin-safe-style-v5')) return;
      const style = document.createElement('style');
      style.id = 'aa-admin-safe-style-v5';
      style.textContent = `
    ...
    function editorHtml(){
      return '<div class="aa-safe-note">Edita esta sección y presiona “Guardar cambios”. Las imágenes se comprimen, se suben y se guardan automáticamente al seleccionarlas.</div>'+ 
    ...
    function injectButton(){
      ...
      if (!hd) return false;
      hd.querySelectorAll('.aa-safe-open-btn:not([data-aa-safe-v5])').forEach(function(old){ old.remove(); });
      if (hd.querySelector('.aa-safe-open-btn[data-aa-safe-v5]')) return true;
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'aa-safe-open-btn';
      btn.setAttribute('data-aa-safe-v5','1');
    ...
    function start(){
      let tries = 0;
      const timer = setInterval(function(){ tries += 1; if (injectButton() || tries > 50) clearInterval(timer); }, 250);
    ...
    console.error('[AdminSafePanelV5] disabled after error:', err);
  }
})();